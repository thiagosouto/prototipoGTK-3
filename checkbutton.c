...
int main (int argc, char *argv[])
{
  ...
  check = gtk_check_button_new_with_label ("Mostrar o titulo");
  gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (check), TRUE);
  g_signal_connect (GTK_TOGGLE_BUTTON (check), "toggled", G_CALLBACK (selecao), janela);
  gtk_container_add (GTK_CONTAINER (caixa), check);

  check = gtk_check_button_new_with_label ("Mostrar o botao 1");
  gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (check), TRUE);
  g_signal_connect (GTK_TOGGLE_BUTTON (check), "toggled", G_CALLBACK (criar_botao), janela);
  gtk_container_add (GTK_CONTAINER (caixa), check);
